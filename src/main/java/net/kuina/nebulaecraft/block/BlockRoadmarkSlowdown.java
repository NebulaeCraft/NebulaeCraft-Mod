...
public class BlockRoadmarkSlowdown extends ElementsNebulaecraftMod.ModElement {
    @GameRegistry.ObjectHolder("nebulaecraft:roadmark_slowdown")
    ...
    public void registerModels(ModelRegistryEvent event) {
        BlockRoadmarkSlowdown.BlockCustom.EnumType[] allSubtypes = BlockRoadmarkSlowdown.BlockCustom.EnumType.values();
        for (BlockRoadmarkSlowdown.BlockCustom.EnumType subtype : allSubtypes) {
            ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), subtype.getMetadata(), new ModelResourceLocation("nebulaecraft:roadmark_slowdown_" + subtype.getName(), "inventory"));
        }
    }
    ...
    public static class BlockCustom extends Block {
        ...
        public BlockCustom() {
            ...
            setUnlocalizedName("roadmark_slowdown");
        ...
        public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
            if (state.getValue(SUBTYPE).getMetadata() == 3) {
                switch (state.getValue(BlockHorizontal.FACING)) {
                    ...
                    case NORTH:
                    default:
                        return new AxisAlignedBB(-0.5, 0, 0.1875, 1.5, 0.1, 0.8125);
                    case EAST:
                    case WEST:
                        return new AxisAlignedBB(0.1875, 0, -0.5, 0.8125, 0.1, 1.5);
                ...
                switch (state.getValue(BlockHorizontal.FACING)) {
                    ...
                    case NORTH:
                    default:
                        return new AxisAlignedBB(0.25, 0, 0, 0.75, 0.1, 1);
                    case EAST:
                    case WEST:
                        return new AxisAlignedBB(0, 0, 0.25, 1, 0.1, 0.75);
...
//            System.out.println(EnumFacing.getFront(facing.getIndex() - 2).getName2());
        ...
        public enum EnumType implements IStringSerializable {
            ...
            private static final BlockRoadmarkSlowdown.BlockCustom.EnumType[] META_LOOKUP = new BlockRoadmarkSlowdown.BlockCustom.EnumType[values().length];

            static {
                for (BlockRoadmarkSlowdown.BlockCustom.EnumType type : values()) {
            ...
            public static BlockRoadmarkSlowdown.BlockCustom.EnumType byMetadata(int meta) {
