...
public class BlockClShieldCorner extends ElementsNebulaecraftMod.ModElement {
    @GameRegistry.ObjectHolder("nebulaecraft:cl_shield_corner")
    public static final Block block = null;

    public BlockClShieldCorner(ElementsNebulaecraftMod instance) {
        super(instance, 115);
    }

    @Override
    public void initElements() {
        elements.blocks.add(() -> new BlockCustom().setRegistryName("cl_shield_corner"));
    ...
    public void registerModels(ModelRegistryEvent event) {
        BlockClShieldCorner.BlockCustom.EnumType[] allSubtypes = BlockClShieldCorner.BlockCustom.EnumType.values();
        for (BlockClShieldCorner.BlockCustom.EnumType subtype : allSubtypes) {
            ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), subtype.getMetadata(), new ModelResourceLocation("nebulaecraft:cl_shield_corner_" + subtype.getName(), "inventory"));
        }
    }
    ...
    public static class BlockCustom extends Block {
        ...
        public BlockCustom() {
			...
			setUnlocalizedName("cl_shield_corner");
			setHardness(1F);
			setResistance(10F);
			setLightLevel(0F);
			setLightOpacity(0);
            ...
            if (state.getValue(SUBTYPE).getMetadata() == 0) {
                switch (state.getValue(BlockHorizontal.FACING)) {
                    case SOUTH :
						...
						case EAST :
							return new AxisAlignedBB(0.375, -0.25, 0, 1, 2, 1);
						case WEST :
                ...
                switch (state.getValue(BlockHorizontal.FACING)) {
                    case SOUTH :
						default :
							return new AxisAlignedBB(-0.25, 0, 0.375, 1, 2, 1);
						case NORTH :
							return new AxisAlignedBB(0, 0, 0, 1.25, 2, 0.625);
						case EAST :
							return new AxisAlignedBB(0.375, 0, 0, 1, 2, 1.25);
						case WEST :
							return new AxisAlignedBB(0, 0, -0.25, 0.625, 2, 1);
        ...
        public enum EnumType implements IStringSerializable {
            ...
            private static final BlockClShieldCorner.BlockCustom.EnumType[] META_LOOKUP = new BlockClShieldCorner.BlockCustom.EnumType[values().length];

            static {
                for (BlockClShieldCorner.BlockCustom.EnumType type : values()) {
            ...
            public static BlockClShieldCorner.BlockCustom.EnumType byMetadata(int meta) {
