...
public class BlockScreenDoorMiddle extends ElementsNebulaecraftMod.ModElement {
    @GameRegistry.ObjectHolder("nebulaecraft:screen_door_middle")
    public static final Block block = null;

    public BlockScreenDoorMiddle(ElementsNebulaecraftMod instance) {
        super(instance, 103);
    }

    @Override
    public void initElements() {
        elements.blocks.add(() -> new BlockCustom().setRegistryName("screen_door_middle"));
    ...
    public void registerModels(ModelRegistryEvent event) {
        BlockScreenDoorMiddle.BlockCustom.EnumType[] allSubtypes = BlockScreenDoorMiddle.BlockCustom.EnumType.values();
        for (BlockScreenDoorMiddle.BlockCustom.EnumType subtype : allSubtypes) {
            ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), subtype.getMetadata(), new ModelResourceLocation("nebulaecraft:screen_door_middle_" + subtype.getName(), "inventory"));
        }
    }
    ...
    public static class BlockCustom extends Block {
        ...
        public BlockCustom() {
            super(Material.IRON);
            setUnlocalizedName("screen_door_middle");
        ...
        public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
            if (state.getValue(SUBTYPE).getMetadata() == 0) {
                ...
                switch (state.getValue(BlockHorizontal.FACING)) {
                    case SOUTH :
						default :
							return new AxisAlignedBB(0, 0, 0.75, 1, 1.375, 1);
						case NORTH :
							return new AxisAlignedBB(0, 0, 0, 1, 1.375, 0.25);
						case EAST :
							return new AxisAlignedBB(0.75, 0, 0, 1, 1.375, 1);
						case WEST :
							return new AxisAlignedBB(0, 0, 0, 0.25, 1.375, 1);
                ...
                switch (state.getValue(BlockHorizontal.FACING)) {
                    case SOUTH :
						default :
							return new AxisAlignedBB(0, 0, 0.75, 1, 1.125, 1);
						case NORTH :
							return new AxisAlignedBB(0, 0, 0, 1, 1.125, 0.25);
						case EAST :
							return new AxisAlignedBB(0.75, 0, 0, 1, 1.125, 1);
						case WEST :
							return new AxisAlignedBB(0, 0, 0, 0.25, 1.125, 1);
        ...
        public enum EnumType implements IStringSerializable {
            ...
            private static final BlockScreenDoorMiddle.BlockCustom.EnumType[] META_LOOKUP = new BlockScreenDoorMiddle.BlockCustom.EnumType[values().length];

            static {
                for (BlockScreenDoorMiddle.BlockCustom.EnumType type : values()) {
            ...
            public static BlockScreenDoorMiddle.BlockCustom.EnumType byMetadata(int meta) {
