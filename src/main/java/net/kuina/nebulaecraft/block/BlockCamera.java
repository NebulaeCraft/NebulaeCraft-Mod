...
public class BlockCamera extends ElementsNebulaecraftMod.ModElement {
    @GameRegistry.ObjectHolder("nebulaecraft:camera")
    public static final Block block = null;

    public BlockCamera(ElementsNebulaecraftMod instance) {
        super(instance, 111);
    }
    ...
    public void registerModels(ModelRegistryEvent event) {
        BlockCamera.BlockCustom.EnumType[] allSubtypes = BlockCamera.BlockCustom.EnumType.values();
        for (BlockCamera.BlockCustom.EnumType subtype : allSubtypes) {
            ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), subtype.getMetadata(), new ModelResourceLocation("nebulaecraft:camera_" + subtype.getName(), "inventory"));
        }
    }
    ...
    public static class BlockCustom extends Block {
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
            if(state.getValue(SUBTYPE).getMetadata()==0){
                ...
                switch (state.getValue(BlockHorizontal.FACING)) {
                    case SOUTH:
                    default:
                        return new AxisAlignedBB(0.25, 0.25, 0, 0.75, 0.75, 0.75);
                    case NORTH:
                        return new AxisAlignedBB(0.25, 0.25, 0.25, 0.75, 0.75, 1);
                    case EAST:
                        return new AxisAlignedBB(0, 0.25, 0.25, 0.75, 0.75, 0.75);
                    case WEST:
        ...
        public enum EnumType implements IStringSerializable {
            ...
            static {
                for (BlockCamera.BlockCustom.EnumType type : values()) {
            ...
            public static BlockCamera.BlockCustom.EnumType byMetadata(int meta) {
