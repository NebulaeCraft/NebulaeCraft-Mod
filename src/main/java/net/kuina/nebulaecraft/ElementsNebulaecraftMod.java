...
public class ElementsNebulaecraftMod implements IFuelHandler, IWorldGenerator {
	...
	public ElementsNebulaecraftMod() {
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == ElementsNebulaecraftMod.ModElement.class)
					elements.add((ElementsNebulaecraftMod.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		this.addNetworkMessage(net.kuina.nebulaecraft.network.PacketAutogenAction.Handler.class,
				net.kuina.nebulaecraft.network.PacketAutogenAction.class, Side.SERVER);
		this.addNetworkMessage(net.kuina.nebulaecraft.network.PacketAutogenGui.Handler.class,
				net.kuina.nebulaecraft.network.PacketAutogenGui.class, Side.CLIENT);
	}
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = NebulaecraftModVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = NebulaecraftModVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				NebulaecraftMod.PACKET_HANDLER.sendTo(new NebulaecraftModVariables.WorldSavedDataSyncMessage(0, mapdata),
						(EntityPlayerMP) event.player);
			if (worlddata != null)
				NebulaecraftMod.PACKET_HANDLER.sendTo(new NebulaecraftModVariables.WorldSavedDataSyncMessage(1, worlddata),
						(EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			NebulaecraftMod.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final ElementsNebulaecraftMod elements;
		protected final int sortid;
		public ModElement(ElementsNebulaecraftMod elements, int sortid) {
